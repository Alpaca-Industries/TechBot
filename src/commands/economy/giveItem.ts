...
export default class giveItemCommand extends Command {
  async messageRun(
    ...
    if (userToGiveTo.success === false || userToGiveTo.value.bot === true)
      return message.reply("Please specify a valid user");
    if (itemToGive.value === null)
      return message.reply("Please specify a valid item");
    if (amount.value < 0 || amount.success === false)
    ...
    const giverInventory = await fetchInventory(
      message.author,
      await fetchItemByName(itemToGive.value)
    );
    const receiverInventory = await fetchInventory(
      userToGiveTo.value,
      await fetchItemByName(itemToGive.value)
    );

    if (giverInventory === undefined)
      return message.reply("You do not have that item");
    if (giverInventory.amount < amount.value)
      return message.reply("You do not have that many of that item");
    ...
    return message.reply(
      `You gave ${amount.value} ${itemToGive.value} to ${userToGiveTo.value.username}`
    );
  }
}
